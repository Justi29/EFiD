...
```
Wyniki symulacji metodą Monte Carlo wskazują, że:
-prognozy logarytmicznych cen są stosunkowo stabilne i oscylują wokół wartości 1.481. Przedziały ufności poszerzają się wraz ze wzrostem horyzontu prognozy, co oznacza większą niepewność w dłuższym okresie,
-prognozy zwykłych cen również wykazują stabilność, utrzymując się w przedziale 4.403-4.408, co sugeruje, że model zakłada minimalne zmiany cen w krótkim okresie.

#Porównanie z prognozami z pkt.3
```{r}
...
```
Rzeczywiste ceny zamknięcia mieszczą się w przedziale od 4.30 do 4.44. Prognozy zwykłych cen uzyskane metodą Monte Carlo są do nich zbliżone. Prognozowane zwykłe ceny z pkt.3 również są zbliżone do rzeczywistych wartości, choć odrobinę wyższe niż te uzyskane za pomocą metody Monte Carlo.  Rzeczywiste logarytmy cen wahają się w przedziale od 1.459 do 1.491 i wykazują pewną zmienność, podczas gdy prognozy logarytmów uzyskane metodą Monte Carlo mieszczą się w zakresie od 1.481 do 1.482, a prognozy z punktu 3 wynoszą 1.47966, co wskazuje na ich stabilność. Modele te mają jednak tendencję do przewidywania mniejszej zmienności niż rzeczywista, co prowadzi do stabilnych prognoz logarytmów i nieco wyższych prognozowanych cen zamknięcia.
