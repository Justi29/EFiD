...
Celem badania jest  wyznaczenie i porównanie prognoz cen oraz stóp zwrotu z uwzględnieniem różnych metod statystycznych.

W tym celu obliczono logarytmy cen i logarytmicznych stóp zwrotu, zbadano stacjonarność, dopasowano model do stóp zwrotu a następnie wysznaczono prognozy, stosując symulacje Monte Carlo oraz bootstrap.
Na końcu przeprowadzono badanie ponownie, uwzględniając występowanie efektu ARCH. 

W badaniu wykorzystano dane spółki BETACOM, ktróra została wybrana samodzielnie z ostatnich 20 spółek znajdujących się w rankingu WIG, gdyż po zostosowaniu wzoru z polecenia, numer ten nie występował już w rankingu. 


## BETACOM 
Dane wykorzystane w projekcie zostały pobrane ze strony *stooq.pl*. Obserwacje dotyczą spółki BETACOM, która działa w branży IT, tworząc oprogramowanie na zamówienie oraz integrując systemy informatyczne.
Dane pochodzą z okresu od 1 września 2023 do 4 października 2024 i zawierają 276 obserwacji, gdzie dla każdej z nich odnotowane są wartości cen otwarcia, zamknięcia, najwyżjesz, najniższej oraz wolumenu.
...
Zgodnie z poleceniem zbiór danych został podzielony na dane treningowe, do końca września 2024 oraz testowe, za które odpowiadały dni 1-4 października 2024. 
```{r}
N <- nrow(new_df) 
...
liczba_treningowych <- nrow(train)
liczba_testowych <- nrow(test)

procent_treningowych <- round((liczba_treningowych / N),3) * 100
procent_testowych <- round((liczba_testowych / N),3) * 100

cat("Procent zbioru treningowego:", procent_treningowych, "%\n")
cat("Procent zbioru testowego:", procent_testowych, "%\n")
```
...
Model nie wykorzystuje przeszłych wartości szeregów czasowych do prognozowania przyszłych wartości, gdyż nie zawiera komponentu autoregresyjnego AR(0,1).
Bieżąca wartość szeregu czasowego jest kombinacją obecnego błędu prognozy oraz wcześniejszego błędu prognozy.
...
```
Na wykresie nie jest widoczne aby poszczególne wartości wykraczały poza progi, co oznacza że nie występuje autokorelacja. 
...
```
Ten sam wniosek wynika również z przeprowadzonego testu Ljunga-Boxa, wartość p-value jest większa od wartości krytycznej 0.05, co oznacza odrzucenie hipotezy zerowej na rzecz hipotezy alternatynwej, mówiącej o tym że nie występuje autokorelacja.  
...
coeftest(model)
```
Parametr średniej ruchomej MA(1) w modelu ARIMA(0,0,1) jest istotny na poziomie 5%, więc w modelu uwzględniamy jedno opóźnienie.
...
Zakładamy rozkład normalny dla logarytmicznych stóp zwrotu, czyli logarytmiczne ceny mają warunkowy rozkład normalmny. Zatem ceny mają rozkład logarytmicznie normalny. To założenie stanowi podstawę dla symulacji przeprowadzonych w kolejnym etapie projektu przy użyciu metody Monte Carlo.
...
```
W obliczeniach do prognozowanania cen wykorzystano wzór na wartość oczekiwaną dla rozkładu log-normalnego:

$$
\text{Wartość oczekiwana} = e^{\mu + \sigma^2 / 2}
$$

Gdzie:
- $\mu$ to prognozowana logarytmiczna wartość ceny,
- $\sigma$ to odchylenie standardowe logarytmicznych cen.
...
```
Prognozy modelu logarytmicznych stóp zwrotu są bliskie zera. Rzeczywiste wartości wykazują większą zmienność, ale mieszczą się w szerokich przedziałach ufności. Szerokie przedziały ufności wskazują na dużą niepewność prognoz. 
Prognozy modelu logarytmicznych cen są zbliżone do wartości 1.48 dla każdej obserwacji. Rzeczywiste wartości wykazują większą zmienność, ale wszystkie mieszczą się w szerokich przedziałach ufności.
Prognozy dla cen zamknięcia są stabilne i zbliżone do wartości 4.418 dla każdej obserwacji, z szerokimi przedziałami ufności, co wskazuje na znaczną niepewność prognoz.
...
```
Model prognozuje ceny zamknięcia oraz logarytmiczne ceny z wynikami zbliżonymi do rzeczywistych, ale występują pewne różnice. Prognozowane ceny zamknięcia (ok. 4.418) są stabilne, podczas gdy rzeczywiste ceny są bardziej zróżnicowane (od 4.30 do 4.44). Podobnie, prognozy logarytmicznych cen są stałe (1.47966) i nie odzwierciedlają w pełni zmienności rzeczywistych wartości (od 1.459 do 1.491).

Prognozowane logarytmiczne stopy zwrotu są bliskie zeru, co sugeruje, że model przewiduje niewielką zmienność. Jednak rzeczywiste stopy zwrotu wykazują większe odchylenia (od -0.032 do 0.023). 
...
Kolejnym etapem projektu było zastosowanie metody Monte Carlo do prognozowania logarytmów cen oraz rzeczywistych cen na kolejne cztery notowania, opierając się na rozkładzie reszt modelu dopasowanego w punkcie 2. Przyjęto, że logarytmiczne stopy zwrotu mają rozkład normalny, co oznacza, że logarytmiczne ceny mają warunkowy rozkład normalny, a rzeczywiste ceny rozkład logarytmicznie normalny. Na podstawie tych symulacji wyznaczono 95% przedziały ufności dla prognozowanych wartości
...
Wyniki symulacji popprzez użycie Monte Carlo przedstawiają prognozy logarytmicznych cen na cztery kolejne notowania wskazuje na stosunkowo stabilne prognozy, które oscylują wokół wartości zbliżonej do 1.481. Przedziały ufności rosną wraz z horyzontem prognozy, co wskazuje na zwiększającą się niepewność w dalszych prognozach. Prognozowane zwykłe ceny są stabilne i oscylują wokół wartości 4.403-4.408. To sugeruje, że model przewiduje niewielkie zmiany w cenach w krótkim okresie.

#Porównanie z pkt.3
```{r}
result_df2 <- as.data.frame(cbind(
test$Zamkniecie,  
ceny_results$Srednia_Ceny, 
predictions$Zamkniecie_pred, 
test$Zamkniecie_log, 
log_ceny_results$Srednia_Log_Ceny,
predictions$Zamkniecie_log
#test$Stopa_zwrotu_log, 
#predictions$Stopa_zwrotu_log_pred
))

colnames(result_df2) <- c("Zamkniecie","Zamkniecie_ceny_MonteCarlo", "Zamkniecie_pred","Zamkniecie_log","Zamkniecie_log_MonteCarlo","Zamkniecie_log_pred")
#"Stopa_zwrotu_log","Stopa_zwrotu_log_pred"

paged_table(result_df2)

```
Rzeczywiste ceny zamknięcia oscylują od 4.30 do 4.44. Prognozy przy zastosowaniu Monte Carlo są zbliżone do rzeczywistych cen zamknięcia. Prognozowane ceny zamknięcie prognozy również są zbliżone do rzeczywistych wartości, choć odrobinę wyższe niż prognozy Monte Carlo.  Rzeczywiste logarytmy cen (od 1.459 do 1.491) wykazują pewną zmienność, ale prognozy logarytmów metodą Monte Carlo (od 1.481 do 1.482) i inne prognozy logarytmów (1.47966) są dość stabilne. Jednak modele te mają tendencję do przewidywania mniejszej zmienności niż ta, która występuje w rzeczywistości, co skutkuje stabilnymi prognozami logarytmów i nieznacznie wyższymi wartościami prognozowanych cen zamknięcia.
...
W wyniku wyznaczonych prognoz logarytmów cen i samych cen za pomocą metod bootstrapowych uzyskano wyniki zbliżone do tych, obliczonych w punktach 3 oraz 4, w szczególności wartości prognozy logarytmów cen z tego sposobu są bardzo zbliżone do logarytmów cen uzyskanych metodą Monte Carlo. 
Również w przypadku prognoz cen, w przypadku największych różnic wyniki różnią się o wartość 0.2 .
...
https://strefainwestorow.pl/notowania/spolki/PLBTCOM00016